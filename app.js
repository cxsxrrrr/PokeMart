...
const DATA_URL = `${API_BASE}/store/cards/`;
const SEARCH_URL = `${API_BASE}/store/cards/search/`;
...
const authModalSuccess = document.getElementById('auth-modal-success');

// ─── Auth State ──────────────────────────────────────────────────────────────
const AUTH_STORAGE_KEY = 'pokemart-auth-user';
...
const updateAuthUI = () => {
    if (currentUser) {
        ...
        authNavBtn.classList.add('auth-nav__btn--logged-in');
        authLoggedOut.hidden = true;
        authLoggedIn.hidden = false;
        authGreeting.textContent = `Hola, ${currentUser.username}`;
        const roleMap = { customer: 'Comprador', seller: 'Vendedor', admin: 'Admin' };
        authRole.textContent = roleMap[currentUser.role] || currentUser.role;
    } else {
        authNavLabel.textContent = 'Iniciar Sesión';
        authNavBtn.classList.remove('auth-nav__btn--logged-in');
...
const checkSession = async () => {
    ...
    try {
        ...
        // If 401 → cookie wasn't sent (file:// protocol), keep localStorage user
    } catch {
        // Server offline — keep localStorage user
    }
};
...
const showAuthError = (msg) => {
    ...
    authModalSuccess.hidden = true;
};

const showAuthSuccess = (msg) => {
    authModalSuccess.textContent = msg;
    authModalSuccess.hidden = false;
...
const setSubmitLoading = (form, loading) => {
    ...
    const text = btn.querySelector('.auth-modal__submit-text');
...
const toggleAuthDropdown = () => {
    const isOpen = authDropdown.classList.contains('is-open');
    if (isOpen) {
        closeAuthDropdown();
    } else {
        authDropdown.classList.add('is-open');
        authDropdown.setAttribute('aria-hidden', 'false');
    }
};
...
const handleRegister = async (e) => {
    ...
    try {
        ...
        if (res.ok || res.status === 201) {
            showAuthSuccess('¡Cuenta creada! Ahora inicia sesión.');
            registerForm.reset();
            setTimeout(() => {
                tabLogin.click();
            }, 1200);
...
const getCardPrimaryImage = (card) => {
    if (typeof card?.image_url === 'string' && card.image_url) {
...
const buildImageCandidates = (card) => {
    if (typeof card?.image_url === 'string' && card.image_url) {
        return [card.image_url, PLACEHOLDER_IMAGE];
    }
    if (!card?.id) {
        return [PLACEHOLDER_IMAGE];
    }
...
const normalizeCard = (card) => {
    const priceValue = toNumber(card?.price ?? card?.recommended_price ?? card?.priceEUR ?? card?.priceUsd);
    const setName = card?.set?.name || card?.setName || card?.collection || 'Colección local';
...
const asegurarCatalogo = async () => {
    ...
    if (!catalogPromise) {
        catalogPromise = fetch(DATA_URL, { cache: 'no-store', credentials: 'include' })
            .then((response) => {
                if (!response.ok) {
...
const buscarCartasEnAPI = async (termino) => {
    const response = await fetch(`${SEARCH_URL}?q=${encodeURIComponent(termino)}`, { credentials: 'include' });
    if (!response.ok) {
...
const cargarCartas = async (termino = '') => {
    ...
    try {
        let filtered;
        if (normalizedTerm) {
            filtered = await buscarCartasEnAPI(normalizedTerm);
        } else {
        ...
        console.error(error);
        mostrarError('No se pudo cargar el catálogo. Asegúrate de que el servidor esté corriendo.');
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (cartCheckoutButton) {
        cartCheckoutButton.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`${API_BASE}/store/orders/create/`, {
                    method: 'POST',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
                });
                ...
                if (response.ok) {
                    ...
                    notifyCartUpdated();
                    closeCartPanel();
                    alert(`¡Orden #${data.id} creada exitosamente! Total: $${data.total_price}`);
                } else {
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') {
            if (authModal.classList.contains('is-open')) {
                closeAuthModal();
            } else if (authDropdown.classList.contains('is-open')) {
                closeAuthDropdown();
            } else if (isCartOpen()) {
                closeCartPanel();
            }
    ...
    openRegisterModalBtn.addEventListener('click', () => openAuthModal('register'));
